...
@implementation HTTPAsyncFileResponseEncrypted
		...
		withPassword:(NSString*)pw
{
	if ((self = [super init]))
	{
		...
		encryptor = [[RNEncryptor alloc] initWithSettings:kRNCryptorAES256Settings
										 password:password
										  handler:^(RNCryptor *cryptor, NSData *dataEncrypted) {
											  DebugLog(@"Decryptor recevied %ld bytes", (unsigned long)dataEncrypted.length);
...
- (BOOL)openFileAndSetupReadSource
{
	...
	readQueue = dispatch_queue_create("HTTPAsyncFileResponseEncrypted", NULL);
